...
public interface ProdutoRepository extends JpaRepository<Produto, Integer> {
	...
	/*/definição de metodo, query coloca o jpql para busca
	@Query("SELECT DISTINCT obj FROM Produto obj INNER JOIN obj.categorias cat WHERE obj.nome LIKE %:nome% AND cat IN :categorias" )
	Page<Produto> search(@Param("nome") String nome, @Param("categorias") List<Categoria> categorias,Pageable pageRequest);//anotação Param substitui o %nome% e IN categorias*/
	
	//mesmo resultado  no metodo acima
